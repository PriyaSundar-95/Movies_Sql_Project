...
select movie_name, concat(revenue-budget) as flop from movies_sql where revenue > budget = 0;


/* 8.Retrieve the names and profit of all the 
Bollywood movies in the
dataset.(profit=revenue – budget)*/
select movie_name, concat(revenue-budget) as profit from movies_sql where revenue < budget = 0;

/* 9.Retrieve the names and loss of all the 
Bollywood movies in the
dataset.(loss=revenue – budget)*/
select movie_name, concat(revenue-budget) as loss from movies_sql where revenue > budget = 0;
...
select movie_name, concat(revenue-budget) as flop from movies_sql where revenue > budget = 0
order by flop desc limit 5;

-- 25. Retrieve the names of top 5 hit movies in the dataset.
select movie_name, concat(revenue-budget) as hit from movies_sql where revenue < budget = 0
...
select movie_name, concat(revenue - budget) as flop from movies_sql where lead_star='akshay kumar' and revenue > budget = 0;

-- 30. With which director Sharukh Khan have given the biggest hit movie .
select director, concat(revenue-budget) as hit from movies_sql where lead_star='shahrukh khan' and revenue < budget = 0
